...
The prototype's CSS class system maps directly to the app's React UI kit in `components/ui/` (exported via `components/ui/index.ts`). When working in the Next.js app, use these React components instead of the raw CSS classes.

| Prototype class(es) | React component | Notes |
|---|---|---|
| `.btn` / `.btn--primary` / `--secondary` / `--ghost` / `--sm` | `components/ui/Button` | `variant` + `size` props mirror modifier names |
| `.eyebrow` / `.eyebrow--feature` | `Eyebrow` | `variant="feature"` for the cyan chip |
| `.card` / `.card--rail` | `Card` | `rail` prop adds the accent→link gradient top-rule |
| `.section` / `.section--band` / `.section-head` | `Section` | `visual="feature"` for the gradient rail + chip; `tone="band"` for the darker strip |
| `.link-cta` / `.link-cta--muted` | `LinkCta` | `muted` prop |
| `.stat` | `Stat` | Metric display block |
| `.signal` | `Signal` | Status display block |
| `.pill` / `.pill--get` / `--post` / `--soon` | `Pill` | `tone` prop |
| `.tag-live` / `.tag-soon` | `Tag` | `tone` prop |
| `.table` / `.table-wrap` | `Table` | Hairline responsive table |
| `.note` / `.sim-note` | `Note` | Sidebar note / caption |
| `.aside-card` | `AsideCard` | Agent/programmatic aside |
| `.code-block` / `.code` / `.code-copy` | `CodeBlock` | Includes chrome + copy button |
| `.field` / `.field-row` / `.field-label` / `.field-hint` / `.field-error` | `Field` | Form field scaffold |
| `.input` / `.input--mono` / `.input-prefix` | `Input` | `mono` prop for address/amount inputs |
| `.segmented` | `Segmented` | Segmented toggle; options via `options` prop |
| `.statusbar` / `--ok` / `--warn` / `--err` / `--info` | `StatusBar` | `tone` prop |
| `.cooldown` | `Cooldown` | Live countdown; companion `formatRemaining` export |
| `#lattice` (hero canvas) | `components/home/LatticeCanvas` | Procedural Clearing Lattice; reduced-motion and DPR-clamped |

All other structural patterns (`.container`, `.prose`, nav chrome, `.docs`, domain modules) are rendered via Next.js layout and page components rather than standalone UI-kit exports.
